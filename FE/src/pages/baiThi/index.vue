<script>
import Layout from "@/layouts/main";
import PageHeader from "@/components/page-header";
import { numeric, required } from "vuelidate/lib/validators";
import appConfig from "@/app.config";
import { pagingModel } from "@/models/pagingModel";

import Multiselect from "vue-multiselect";
import { baiThiModel } from "@/models/baiThiModel";
import DatePicker from "vue2-datepicker";
export default {
  page: {
    title: "Quản lý nhà đất",
    meta: [{ name: "description", content: appConfig.description }],
  },
  components: { Layout, PageHeader, Multiselect, DatePicker },
  data() {
    return {
      title: "Thông tin nhà đất",
      items: [
        {
          text: "Tài khoản",
          href: "/tai-khoan",
        },
        {
          text: "Danh sách",
          active: true,
        },
      ],
      data: [],
      fields: [
        {
          key: "STT",
          label: "STT",
          class: "td-stt",
          sortable: false,
          thClass: "hidden-sortable",
        },
        {
          key: "name",
          label: "Tên nền",
          class: "td-tennen",
          sortable: true,
          thStyle: "text-align:center",
          thClass: "hidden-sortable",
        },
        {
          key: "dienTich",
          label: "Thông tin ",
          class: "td-thongtin",
          sortable: true,
          thStyle: "text-align:center",
          thClass: "hidden-sortable",
        },
        // {
        //   key: "giaTriNen",
        //   label: "Giá trị nền(VNĐ) ",
        //   class: "td-ten",
        //   sortable: true,
        //   thStyle: "text-align:center",
        //   thClass: "hidden-sortable",
        // },
        {
          key: "diaChi",
          label: "Địa chỉ ",
          class: "td-diachi",
          sortable: true,
          thStyle: "text-align:center",
          thClass: "hidden-sortable",
        },
        // {
        //   key: "ghiChu",
        //   label: "Ghi chú",
        //   class: "td-ten",
        //   sortable: true,
        //   thStyle: "text-align:center",
        //   thClass: "hidden-sortable",
        // },

        {
          key: "trangThai",
          label: "Trạng thái",
          class: "td-trangthai",
          thStyle: "text-align:center",
          sortable: true,
          thClass: "hidden-sortable",
        },
        // {
        //   key: "ngayKyHopDongShow",
        //   label: "Ngày ký hợp đồng",
        //   class: "td-email",
        //   thStyle: "text-align:center",
        //   sortable: true,
        //   thClass: "hidden-sortable",
        // },
        {
          key: "process",
          label: "Xử lý",
          class: "td-xuly",
          sortable: false,
          thClass: "hidden-sortable",
        },
      ],
      currentPage: 1,
      perPage: 10,
      pageOptions: [5, 10, 25, 50, 100],
      showModal: false,
      showDeleteModal: false,
      submitted: false,
      sortBy: "age",
      filter: null,
      sortDesc: false,
      filterOn: [],
      numberOfElement: 1,
      totalRows: 1,
      model: baiThiModel.baseJson(),
      listTrangThai: [],

      itemFilter: {
        content: null,
        trangThaiId: null,
      },
      pagination: pagingModel.baseJson(),
    };
  },
  created() {
    this.getListTrangThai();
  },
  validations: {
    model: {
      name: { required },
      diaChi: { required },
      giaTriNen: { required },
      dienTich: { required },
      trangThai: { required },
    },
  },

  computed: {
    isStatusOne() {
      return (
        this.model.trangThai &&
        this.model.trangThai.id === "665d1e11a3737cb469025205"
      );
    },
  },
  methods: {
    async fnGetList() {
      this.$refs.tblList?.refresh();
    },
    async getListTrangThai() {
      await this.$store
        .dispatch("commonStore/getAll", "DM_TRANGTHAI")
        .then((res) => {
          this.listTrangThai = res.data || [];
        });
    },

    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store
          .dispatch("baiThiStore/delete", { id: this.model.id })
          .then((res) => {
            if (res.code === 0) {
              this.fnGetList();
              this.showDeleteModal = false;
            }
            var a = {
              message: res.message,
              code: res.code,
            };
            this.$store.dispatch("snackBarStore/addNotify", {
              message: res.message,
              code: res.code,
            });
          });
      }
    },
    handleShowDeleteModal(id) {
      this.model.id = id;
      this.showDeleteModal = true;
    },
    async handleSubmit(e) {
      e.preventDefault();
      this.submitted = true;
      this.$v.$touch();
      if (this.$v.$invalid) {
        return;
      } else {
        let loader = this.$loading.show({
          container: this.$refs.formContainer,
        });
        if (this.model.id != 0 && this.model.id != null && this.model.id) {
          // Update model
          if(this.model.trangThai.id == "665d1e03a3737cb469025204"){
            this.model.tenNguoiMua = null;
            this.model.sdt = null;
            this.model.ngayKyHopDong = null;
            this.model.cmnd = null;
            this.$refs.tblList.refresh();
          }
          await this.$store
            .dispatch("baiThiStore/update", this.model)
            .then((res) => {
              if (res.code === 0) {
                this.showModal = false;
                this.$refs.tblList.refresh();
                
              }
              this.$store.dispatch("snackBarStore/addNotify", {
                message: res.message,
                code: res.code,
              });
            });
        } else {
          // Create model
          await this.$store
            .dispatch("baiThiStore/create", this.model)
            .then((res) => {
              if (res.code === 0) {
                this.fnGetList();
                this.showModal = false;
                this.model = {};
              }
              this.$store.dispatch("snackBarStore/addNotify", {
                message: res.message,
                code: res.code,
              });
            });
        }
        loader.hide();
      }
      this.submitted = false;
    },
    async handleUpdate(id) {
      await this.$store
        .dispatch("baiThiStore/getById", { id: id })
        .then((res) => {
          if (res.code === 0) {
            const data = baiThiModel.fromJson(res.data);
            this.model = {
              ...data,
              ngayKyHopDong: data.ngayKyHopDong ? new Date(data.ngayKyHopDong) : null,
            };
            this.showModal = true;
          } else {
            this.$store.dispatch("snackBarStore/addNotify", {
              message: res.message,
              code: res.code,
            });
          }
        });
    },
    myProvider(ctx) {
      const params = {
        start: ctx.currentPage,
        limit: ctx.perPage,
        content: this.itemFilter.content,
        trangThaiId: this.itemFilter.trangThaiId?.id,
        startDate: this.itemFilter.ngayBatDau
          ? new Date(this.itemFilter.ngayBatDau)
          : null,
        endDate: this.itemFilter.ngayKetThuc
          ? new Date(this.itemFilter.ngayKetThuc)
          : null,
        sortDesc: ctx.sortDesc,
      };
      this.loading = true;
      try {
        let promise = this.$store.dispatch(
          "baiThiStore/getPagingParams",
          params
        );
        return promise.then((resp) => {
          let items = resp.data.data;
          this.totalRows = resp.data.totalRows;
          this.numberOfElement = resp.data.data.length;
          this.loading = false;
          return items || [];
        });
      } finally {
        this.loading = false;
      }
    },
    addCoQuanToModel(node, instanceId) {
      if (node.id) {
        this.itemFilter.trangThaiId = node.id;
      }
    },
    handleSearch() {
      this.$refs.tblList.refresh();
    },
    handleClear() {
      this.itemFilter = {
        content: null,

        trangThaiId: null,
      };
      this.$refs.tblList.refresh();
    },
  },
  mounted() {},
  watch: {
    model: {
      deep: true,
      handler(val) {
        this.addCoQuanToModel(val);
        // this.saveValueToLocalStorage()
      },
    },
    showModal(status) {
      if (status == false) this.model = baiThiModel.baseJson();
    },
    // model() {
    //   return this.model;
    // },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
      }
    },
  },
};
</script>

<template>
  <Layout>
    <PageHeader :title="title" :items="items" />
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <div class="row mb-2">
              <div class="mb-6 col-lg-6">
                <label>Nội dung</label>
                <input
                  v-model="itemFilter.content"
                  type="text"
                  name="untyped-input"
                  class="form-control"
                  placeholder="Tìm kiếm theo tiêu đề"
                  style="height: 39px"
                />
              </div>

              <div class="mb-6 col-lg-6">
                <label>Trạng thái</label>
                <multiselect
                  v-model="itemFilter.trangThaiId"
                  :options="listTrangThai"
                  label="name"
                  :multiple="false"
                  :value="itemFilter.trangThaiId"
                  selectLabel="Nhấn vào để chọn lô đất"
                  deselectLabel="Nhấn vào để xóa lô đất"
                  placeholder="Chọn lô đất"
                >
                </multiselect>
              </div>

              <div class="col-sm-12">
                <div class="text-sm-end">
                  <b-modal
                    v-model="showModal"
                    title="Thông tin nhà đất"
                    title-class="text-black font-18"
                    body-class="p-3"
                    hide-footer
                    centered
                    no-close-on-backdrop
                    size="lg"
                  >
                    <form @submit.prevent="handleSubmit" ref="formContainer">
                      <div class="row">
                        <div class="col-4">
                          <div class="mb-3">
                            <label class="text-left">Tên nền</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="userName"
                              v-model.trim="model.name"
                              type="text"
                              class="form-control"
                              placeholder="Nhập vào tên nền"
                              :class="{
                                'is-invalid': submitted && $v.model.name.$error,
                              }"
                            />
                            <div
                              v-if="submitted && !$v.model.name.required"
                              class="invalid-feedback"
                            >
                              Tên nền không được trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-4">
                          <div class="mb-3">
                            <label class="text-left">Diện tích</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="lastName"
                              v-model="model.dienTich"
                              type="number"
                              class="form-control"
                              placeholder="Nhập diện tích"
                              :class="{
                                'is-invalid':
                                  submitted && $v.model.dienTich.$error,
                              }"
                            />
                            <div
                              v-if="submitted && !$v.model.dienTich.required"
                              class="invalid-feedback"
                            >
                              Diện tích không được trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-4">
                          <div class="mb-3">
                            <label class="text-left">Giá trị nền</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="lastName"
                              v-model="model.giaTriNen"
                              type="number"
                              class="form-control"
                              placeholder="Nhập giá trị nền"
                              :class="{
                                'is-invalid':
                                  submitted && $v.model.giaTriNen.$error,
                              }"
                            />
                            <div
                              v-if="submitted && !$v.model.giaTriNen.required"
                              class="invalid-feedback"
                            >
                              Giá trị nền không được trống
                            </div>
                          </div>
                        </div>

                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Địa chỉ nền nhà</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="userName"
                              v-model.trim="model.diaChi"
                              type="text"
                              class="form-control"
                              placeholder="Nhập vào địa chỉ"
                              :class="{
                                'is-invalid':
                                  submitted && $v.model.diaChi.$error,
                              }"
                            />
                            <div
                              v-if="submitted && !$v.model.name.required"
                              class="invalid-feedback"
                            >
                              Tên doanh nghiệp không được trống.
                            </div>
                          </div>
                        </div>

                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Ghi chú</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="userName"
                              v-model.trim="model.ghiChu"
                              type="text"
                              class="form-control"
                              placeholder="Nhập vào ghi chú"
                            />
                          </div>
                        </div>
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Trạng thái</label>
                            <span style="color: red">&nbsp;*</span>
                            <multiselect
                              v-model="model.trangThai"
                              :options="listTrangThai"
                              label="name"
                              :multiple="false"
                              :value="model.trangThai"
                              selectLabel="Nhấn vào để chọn trạng thái"
                              deselectLabel="Nhấn vào để xóa trạng thái"
                              placeholder="Chọn trạng thái"
                            >
                            </multiselect>
                          </div>
                        </div>
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Tên người mua</label>
                            <!-- <span style="color: red">&nbsp;*</span> -->
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="userName"
                              v-model.trim="model.tenNguoiMua"
                              type="text"
                              class="form-control"
                              :disabled="!isStatusOne"
                              placeholder="Nhập vào tên người mua"
                            />
                          </div>
                        </div>
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Số điện thoại</label>
                            <!-- <span style="color: red">&nbsp;*</span> -->
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="lastName"
                              v-model="model.sdt"
                              type="number"
                              class="form-control"
                              :disabled="!isStatusOne"
                              placeholder="Nhập diện tích"
                              maxlength="10"
                              
                            />
                            
                          </div>
                        </div>

                        <div class="col-6">
                          <div class="mb-3">
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id" />

                            <label>Thời gian bắt đầu</label>
                            <date-picker
                              v-model="model.ngayKyHopDong"
                              format="DD/MM/YYYY"
                              :disabled="!isStatusOne"
                            ></date-picker>
                          </div>
                        </div>

                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">CMND/CCCD</label>
                            <!-- <span style="color: red">&nbsp;*</span> -->
                            <input type="hidden" v-model="model.id" />
                            <input
                              id="lastName"
                              v-model="model.cmnd"
                              type="text"
                              class="form-control"
                              :disabled="!isStatusOne"
                              placeholder="Nhập giá trị nền"
                            />
                          </div>
                        </div>
                      </div>
                      <div class="text-end pt-2 mt-3">
                        <b-button
                          variant="light"
                          @click="showModal = false"
                          class="border-0"
                        >
                          Đóng
                        </b-button>
                        <b-button
                          type="submit"
                          variant="success"
                          class="ms-1 cs-btn"
                          >Lưu
                        </b-button>
                      </div>
                    </form>
                  </b-modal>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-12 text-center">
                <b-button
                  variant=""
                  class="w-10 btn-search"
                  style="
                    margin-right: 10px;
                    height: 40px;
                    width: 130px;
                    font-size: 14px;
                    background-color: #e9ab2e;
                    border: none;
                    color: #000 !important;
                    font-weight: bold;
                  "
                  size="sm"
                  @click="handleSearch"
                >
                  <i class="bx bx-search font-size-16 align-middle me-2"></i>
                  Tìm kiếm
                </b-button>
                <b-button
                  class="w-10 btn-reset"
                  style="
                    height: 40px;
                    width: 130px;
                    font-size: 14px;
                    background-color: #e9ab2e;
                    border: none;
                    color: #000 !important;
                    font-weight: bold;
                  "
                  size="sm"
                  @click="handleClear"
                >
                  <i class="mdi mdi-replay font-size-16 align-middle"></i>
                  Làm mới
                </b-button>
              </div>
            </div>
            <div class="row">
              <div class="col-12">
                <div class="row mt-4">
                  <div class="col-sm-12 col-md-12">
                    <div
                      class="col-md-12"
                      style="display: flex; justify-content: space-between"
                    >
                      <div
                        id="tickets-table_length"
                        class="dataTables_length m-1"
                        style="
                          display: flex;
                          justify-content: left;
                          align-items: center;
                        "
                      >
                        <div class="me-1">Hiển thị</div>
                        <b-form-select
                          class="form-select form-select-sm"
                          v-model="perPage"
                          size="sm"
                          :options="pageOptions"
                          style="width: 70px"
                        ></b-form-select
                        >&nbsp;
                        <div style="width: 100px">dòng</div>
                      </div>
                      <b-button
                        type="button"
                        class="btn-label cs-btn-success mb-2 me-2 css-button"
                        @click="showModal = true"
                        size="md"
                        style="border: none;"
                      >
                        <i class="mdi mdi-plus-box label-icon"></i> Thêm
                      </b-button>
                    </div>
                  </div>
                </div>
                <div class="table-responsive mb-0">
                  <b-table
                    class="datatables table-admin"
                    :items="myProvider"
                    :fields="fields"
                    striped
                    bordered
                    responsive="sm"
                    :per-page="perPage"
                    :current-page="currentPage"
                    :sort-by.sync="sortBy"
                    :sort-desc.sync="sortDesc"
                    :filter="filter"
                    :filter-included-fields="filterOn"
                    ref="tblList"
                    primary-key="id"
                  >
                    <template v-slot:cell(STT)="data">
                      {{ data.index + (currentPage - 1) * perPage + 1 }}
                    </template>

                    <template v-slot:cell(trangThai)="data">
                      <div v-if="data.item.trangThai.id = '665d1e11a3737cb469025205' ">
                        <span style="background-color: #e9ab2e45; padding: 5px; border-radius: 5px; color: #e9ab2e;">
                          {{ data.item.trangThai?.name }}
                        </span>
                      </div>
                      <div v-else-if="data.item.trangThai.id = '665d1e11a3737cb469025204'">
                        <span style="background-color: #f4433644;">
                          {{ data.item.trangThai?.name }}
                        </span>
                      </div>
                    </template>
                    <template v-slot:cell(dienTich)="data">
                      <p style="margin-left: 5px; margin-bottom: 5px;">
                        Diện tích: <strong>{{ data.item.dienTich }}</strong> (m2)
                      </p>
                      <p style="margin-left: 5px; margin-bottom: 5px;">
                        Giá trị: <strong>{{ data.item.giaTriNen }}</strong> (VNĐ)
                      </p>
                    </template>
                    

                    <template v-slot:cell(process)="data">
                      <button
                        type="button"
                        size="sm"
                        class="btn btn-outline btn-sm"
                        v-on:click="handleUpdate(data.item.id)"
                      >
                        <i class="fas fa-pencil-alt text-success me-1"></i>
                      </button>
                      <button
                        type="button"
                        size="sm"
                        class="btn btn-outline btn-sm"
                        v-on:click="handleShowDeleteModal(data.item.id)"
                      >
                        <i class="fas fa-trash-alt text-danger me-1"></i>
                      </button>
                    </template>
                  </b-table>
                </div>
                <div class="row mt-3">
                  <b-col>
                    <div>
                      Hiển thị {{ numberOfElement }} trên tổng số
                      {{ totalRows }} dòng
                    </div>
                  </b-col>
                  <div class="col">
                    <div
                      class="dataTables_paginate paging_simple_numbers float-end"
                    >
                      <ul class="pagination pagination-rounded mb-0">
                        <!-- pagination -->
                        <b-pagination
                          v-model="currentPage"
                          :total-rows="totalRows"
                          :per-page="perPage"
                        ></b-pagination>
                      </ul>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <b-modal
              v-model="showDeleteModal"
              centered
              title="Xóa dữ liệu"
              title-class="font-18"
              no-close-on-backdrop
            >
              <p>Dữ liệu xóa sẽ không được phục hồi!</p>
              <template #modal-footer>
                <button
                  v-b-modal.modal-close_visit
                  class="btn btn-outline-info m-1"
                  v-on:click="showDeleteModal = false"
                >
                  Đóng
                </button>
                <button
                  v-b-modal.modal-close_visit
                  class="btn btn-danger btn m-1"
                  v-on:click="handleDelete"
                >
                  Xóa
                </button>
              </template>
            </b-modal>
          </div>
        </div>
      </div>
    </div>
  </Layout>
</template>
<style>

</style>
